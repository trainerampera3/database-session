...
# Import ETL preprocessing functions
import sys
...
def load_job_dataframe(job_id: str, filename: str) -> pd.DataFrame:
...
async def upload_csv(file: UploadFile = File(...)):
    ...
    original_path = job_directory / "original.csv"
...
def clean_data(job_id: str, request: CleaningRequest):
    df = load_job_dataframe(job_id, "original.csv")
    ...
    cleaned_path = get_job_directory(job_id) / "cleaned.csv"
    ...
    return {
        ...
        .astype(object)
        .where(pd.notna(cleaned_df.head(20)), None)
        .to_dict(orient="records"),
    }


@router.post("/{job_id}/transform")
def transform_data(job_id: str, request: TransformationRequest):
    cleaned_path = get_job_directory(job_id) / "cleaned.csv"

    if not cleaned_path.exists():
        raise HTTPException(
            status_code=400,
            detail="Please clean the dataset before applying transformations.",
        )

    df = pd.read_csv(cleaned_path, sep=None, engine="python")
    ...
    transformed_path = get_job_directory(job_id) / "transformed.csv"
    transformed_df.to_csv(transformed_path, index=False)

    profile = profile_dataframe(transformed_df)

    return {
        "success": True,
        "job_id": job_id,
        "profile": profile,
        "columns": [str(column) for column in transformed_df.columns],
        "preview": transformed_df.head(20)
        .astype(object)
        .where(pd.notna(transformed_df.head(20)), None)
        .to_dict(orient="records"),
    }
...
def validate_dataset(job_id: str):
    ...
    if not transformed_path.exists():
        raise HTTPException(
            status_code=400,
            detail="Please transform the dataset before validation.",
        )

    try:
        df = pd.read_csv(transformed_path, sep=None, engine="python")
...
def migrate_data(job_id: str, request: MigrationRequest):
    transformed_path = get_job_directory(job_id) / "transformed.csv"

    if not transformed_path.exists():
        raise HTTPException(
            status_code=400,
            detail="Please transform the dataset before migration.",
        )

    df = pd.read_csv(transformed_path, sep=None, engine="python")

    try:
        rows_migrated = migrate_dataframe(df, request.table_name)
    except Exception as exc:
        raise HTTPException(
            status_code=500,
